...
void FastText::loadModel(std::istream& in) {
  ...
  if (args_->model == model_name::sup) {
    ...
    model_->setTargetCounts(dict_->getCounts(entry_type::word));
        ...
        std::cerr << args_->endjamo << args_->endchar << args_->maxjn << std::endl;
}
...
real FastText::cos(const Vector& queryVec, const Vector& answerVec) {
  real queryNorm = queryVec.norm();
  real answerNorm = answerVec.norm();
  Vector vec1(args_->dim), vec2(args_->dim);
  vec1.zero();
  vec1.addVector(queryVec, 1.0 / (queryNorm+1e-8));
  vec2.zero();
  vec2.addVector(answerVec, 1.0 / (answerNorm+1e-8));

  real score = vec1.dot(vec2);
  return score;
}
...
void FastText::analogy_test(std::istream& in, int32_t k) {
  int32_t nexamples = 0, nlabels = 0;
  double distance = 0.0;
  std::vector<std::string> words;
  Vector query(args_->dim), answer(args_->dim);
  Vector vec(args_->dim);
  while (in.peek() != EOF) {
    dict_->getLine(in, words, model_->rng);
    if (words.size() > 3) {
      query.zero();
      answer.zero();
      getWordVector(vec, words[0]);
      query.addVector(vec, -1.0/(vec.norm()+1e-8));
      getWordVector(vec, words[1]);
      query.addVector(vec, 1.0/(vec.norm()+1e-8));
      getWordVector(vec, words[2]);
      query.addVector(vec, 1.0/(vec.norm()+1e-8));
      
      getWordVector(vec, words[3]);
      answer.addVector(vec, 1.0/(vec.norm()+1e-8));

      distance += 1-cos(query, answer);
      nexamples++;
      if (nexamples % 100 == 0) {
        std::cerr << words[0] << " " << words[1] << " " << words[2] << " | " << words[3] << "\t" << query << " " << answer << std::endl;
      }
      
      if (nexamples%1000==0){
        std::string category;
        switch(nexamples){
          case 1000: category="Capt"; break;
          case 2000: category="Gend"; break;
          case 3000: category="Name"; break;
          case 4000: category="Lang"; break;
          case 5000: category="Misc"; break;
          case 6000: category="Case"; break;
          case 7000: category="Tense"; break;
          case 8000: category="Voice"; break;
          case 9000: category="Form"; break;
          case 10000: category="Honr"; break;
        }
        
        std::cerr << category << "\t" << distance / 1000 << std::endl;      
        distance = 0;
      }
...
void FastText::extract_vec(std::istream& in, std::string out) {
  std::vector<std::string> words;
  Vector vec(args_->dim);

  std::ofstream ofs(out + "_ext.vec");
  if (!ofs.is_open()) {
    std::cerr << "Error opening file for saving vectors." << std::endl;
    exit(EXIT_FAILURE);
  }
...
void FastText::get_analogy_vecs(std::istream& in, std::string out) {
  std::ofstream ofs(out + ".vec");
  int32_t nexamples = 0, nlabels = 0;
  double distance = 0.0;
  std::vector<std::string> words;
  Vector query(args_->dim), answer(args_->dim);
  ...
  while (in.peek() != EOF) {
    ...
    if (words.size() > 3) {
      ...
      ofs << words[0] << " " << words[1] << " " <<  words[2] << " " << query << std::endl;
