...
void Args::parseArgs(const std::vector<std::string>& args) {
  ...
  while (ai < args.size()) {
    ...
    } else if (args[ai] == "-endchar") {
      endchar = std::string(args[ai + 1]);
    } else if (args[ai] == "-endjamo") {
      endjamo = std::string(args[ai + 1]);
    } else if (args[ai] == "-lr") {
    ...
    } else if (args[ai] == "-minsc") {
      minsc = std::stoi(args[ai + 1]);
    } else if (args[ai] == "-maxsc") {
      maxsc = std::stoi(args[ai + 1]);
...
void Args::printDictionaryHelp() {
  std::cerr
    ...
    << "  -minsc              min length of subchar ngram [" << minsc << "]\n"
    << "  -maxsc              max length of subchar ngram [" << maxsc << "]\n"
    << "  -endchar            end character symbol. (type:char) [" << endchar << "]\n"
    << "  -endjamo            end jamo symbol. (type:char) [" << endjamo << "]\n"
...
void Args::save(std::ostream& out) {
  ...
  out.write((char*) &(minsc), sizeof(int));
  out.write((char*) &(maxsc), sizeof(int));
...
void Args::load(std::istream& in) {
  ...
  in.read((char*) &(minsc), sizeof(int));
  in.read((char*) &(maxsc), sizeof(int));
