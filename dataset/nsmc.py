...
def main():
    ...
    if vocab == "pretrain":
        ...
        # error
        raise ValueError("Not implemented for vocab other than 'pretrain'")
    ...
    train_data = pd.read_csv(
        f"./data/parsed_sent_analysis/train_{train_code}.txt",
        header=0,
        delimiter="\t",
        quoting=3,
    )
    dev_data = pd.read_csv(
        f"./data/parsed_sent_analysis/dev_{train_code}.txt",
        header=0,
        delimiter="\t",
        quoting=3,
    )
    test_data = pd.read_csv(
        f"./data/parsed_sent_analysis/test_{train_code}.txt",
        header=0,
        delimiter="\t",
        quoting=3,
    )
    ...
    for seed in random_seeds:
        ...
        _loss, _acc, _precision, _recall, _f1score = model.evaluate(
    ...
    print(
        "accuracy: {:.2f}%, precision: {:.3f}, recall: {:.3f}, f1score: {:.3f}, f1_average: {:.3f}".format(
            acc_total, precision_total, recall_total, f1, f1_total
        )
    )
    print()
